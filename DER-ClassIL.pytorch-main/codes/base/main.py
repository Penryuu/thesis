...
# repo_name = 'humantestDER.pytorch'
...
def main():
    ...
    def train(_run, _rnd, _seed):
        ...
        print(f"Peak memory usage: ",tracemalloc.get_traced_memory())
        ex.logger.info("memory stats: ")
        ex.logger.info(tracemalloc.get_traced_memory())
        ex.logger.info("peak memory used: ")
        ex.logger.info(mem_usage)
        tracemalloc.stop()
    ...
    def _train(cfg, _run, ex, tensorboard):
        ...
        ex.logger.info("result template: ")
        results = results_utils.get_template_results(cfg)
        ex.logger.info(results)
        ex.logger.info("inc_dataset.n_tasks: ")
        ex.logger.info(inc_dataset.n_tasks)
