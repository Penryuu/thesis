...
class IncrementalDataset:
    ...
    def _setup_data(self, dataset):
        ...
        ##MY CODE
        current_class_idx = 0
        train_root_dir = 'C:\Thesis Code\\train'
        train_dataset = dataset(root=train_root_dir)
        test_root_dir = 'C:\Thesis Code\\test'
        test_dataset = dataset(root=test_root_dir)

        X_TDEBUG = np.array(train_dataset.data)
        Y_TDEBUG = np.array(train_dataset.targets)
        print('X_TDEBUG.shape = ',X_TDEBUG.shape)
        print('Y_TDEBUG = ',Y_TDEBUG)
        X_VDEBUG = np.array(test_dataset.data)
        Y_VDEBUG = np.array(test_dataset.targets)
        print('X_VDEBUG.shape = ',X_VDEBUG.shape)
        print('Y_VDEBUG = ',Y_VDEBUG)
        Class_orderDEBUG = dataset.class_order
        print('Class_orderDEBUG = ',Class_orderDEBUG)        

        # root_dir = 'Z:\ProposalAlgos\humandetectiondataset\eval'
        # evalset = dataset(root=root_dir, transform=transform_test)
        self.train_dataset = train_dataset
        self.test_datasets = test_dataset
        self.n_tot_cls = 2  #number of classes in whole dataset
        ##END OF MY CODE

        self._setup_data_for_raw_data(dataset, train_dataset, test_dataset, current_class_idx)
        #self._setup_data_for_raw_data(dataset, current_class_idx)
        # !list
    ...
    def _get_loader(self, x, y, share_memory=None, shuffle=True, mode="train", batch_size=None, resample=None):
        ...
        if "train" in mode:
            ...
            if resample_ is False:
                ...
                sampler = WeightedRandomSampler(y)
