...
def compute_confusion_matrix(tg_model, tg_feature_model, class_means, evalloader, print_info=False, device=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(evalloader):
            ...
            #all_predicted = all_predicted.cpu()

            outputs_feature = np.squeeze(tg_feature_model(inputs))
            outputs_feature = outputs_feature.cpu()
            classmeansT = class_means[:,:,0].T
            classmeansT = torch.from_numpy(classmeansT)
            classmeansT = classmeansT.cpu()
            # Compute score for iCaRL
            sqd_icarl = cdist(classmeansT, outputs_feature, 'sqeuclidean')
            ...
            predicted_icarl = predicted_icarl.cpu()
            all_predicted_icarl.append(predicted_icarl)
            #all_predicted_icarl = all_predicted_icarl.cpu()
            # Compute score for NCM

            sqd_ncm = cdist(classmeansT, outputs_feature, 'sqeuclidean')
            ...
            predicted_ncm = predicted_ncm.cpu()
            all_predicted_ncm.append(predicted_ncm)
            targets = targets.cpu()
            all_targets.append(targets)
            #all_predicted_ncm = all_predicted_ncm.cpu()
