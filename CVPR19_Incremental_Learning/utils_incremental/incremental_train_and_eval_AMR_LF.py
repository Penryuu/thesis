    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if iteration == start_iteration:
                ...
                gt_index = gt_index.scatter(1, targets.reshape(-1,1), 1).ge(0.5)
                ...
                none_gt_index = none_gt_index.scatter(1, targets.reshape(-1,1), 1).le(0.5)
                ...
                if  hard_num > 0:
                    gt_scores = gt_scores[hard_index].reshape(-1, 1).repeat(1, K)
                    ...
                    loss3 = nn.MarginRankingLoss(margin=dist)(gt_scores.reshape(-1, 1), \
                        hard_scores.reshape(-1, 1), torch.ones(hard_num*K).to(device)) * lw_mr
                else:
