...
parser.add_argument('--dataset', default='humandetection', type=str)
...
parser.add_argument('--nb_protos', default=1000, type=int, \
...
lr_strat               = [35, 70]      # Epochs where learning rate gets decreased
...
#Evalset
import evaluationdata as evd

train_root_dir = 'C:\Thesis Code\\train'
trainset = evd.CustomDataset(train_root_dir, transform=transform_train)
test_root_dir = 'C:\Thesis Code\\test'
testset = evd.CustomDataset(test_root_dir, transform=transform_test)
root_dir = 'C:\Thesis Code\\test'
evalset = evd.CustomDataset(root_dir, transform=transform_test)

##END OF MY CODE

# Initialization
dictionary_size     = 4717
...
print('X_valid_total.shape = ',X_valid_total.shape)
...
for iteration_total in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        if iteration == start_iter:
            ...
            #tg_model = tg_model.type(torch.LongTensor)
            ref_model = None
        else:
            ############################################################
            last_iter = iteration
...
print(f"memory stats: ",tracemalloc.get_traced_memory())
