...
parser.add_argument('--order', \
    default='./checkpoint/humandetection_order_run_0.pkl', \
    type=str)
...
#Evalset
# train_root_dir = 'Z:\ProposalAlgos\humandetectiondataset\\train1'
# trainset = evd.CustomDataset(root=train_root_dir, transform=transform_train)
# test_root_dir = 'Z:\ProposalAlgos\humandetectiondataset\\test1'
# testset = evd.CustomDataset(root=test_root_dir, transform=transform_test)
root_dir = 'Z:\ProposalAlgos\humandetectiondataset\\test'
evalset = evd.CustomDataset(root=root_dir, transform=transform_test)
#END OF MY CODE
...
for iteration in range(start_iter, int(100/nb_cl)):
    ...
    #print('Max and Min of valid labels: {}, {}'.format(min(evalset.targets), max(evalset.targets)))
    evalloader = torch.utils.data.DataLoader(evalset, batch_size=128,
        shuffle=False, num_workers=0)
